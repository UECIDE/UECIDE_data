...
/*******************************************************************************
  ...
  Example:
    <code>
    if ( UART_SUCCESS != UARTConfigure(UART1, UART_RTS_WHEN_RX_NOT_FULL|UART_ENABLE_PINS_CTS_RTS) )
    {
...
/*******************************************************************************
  ...
  Example:
    <code>
    if ( != UARTSetFifoMode(UART1, UART_INTERRUPT_ON_TX_DONE|UART_INTERRUPT_ON_RX_NOT_EMPTY) )
    {
        DBPRINT("UART1 invalid FIFO mode!\n");
    }
    </code>
...
/*******************************************************************************
  ...
  Example:
    ...
    actualDataRate = UARTSetDataRate(UART1, GetPeripheralClock(), DESIRED_DATA_RATE);
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    currentRate = UARTGetDataRate(UART1, GetPeripheralClock());
    if ( abs(currentRate-UART1A_DESIRED_DATA_RATE) > (UART1A_DESIRED_DATA_RATE/10) )
    {
        DBPRINT("UART1 data rate (%d) error exceeds 10\%\n", currentRate);
    }
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    UARTSetAddress(UART1, IEEE_485_SLAVE_ADDRESS, TRUE);
    </code>
