...
static __inline__ __attribute__((always_inline)) unsigned int __NVMProgram(void * address, const void * data, unsigned int size, void* pagebuff)
{
  __asm__ volatile (" .equ ___memcpyaddr, memcpy \n");
  return NVMProgram(address, data, size, pagebuff);
}
#undef NVMProgram
#define NVMProgram __NVMProgram
#endif
